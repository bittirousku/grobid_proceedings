...
FILE_SEARCH_PATTERN = [
    ...
    #('cnum with Proceedings suffixed with optional dot',
     ...
     #('773__w', '980__a')),
    # Example: C08-02-18_1-35_new_physics_and_the_lhc.pdf
    ('cnum, page range, and title',
     re.compile(r'^(C\d\d-\d\d-\d\d)_(\d+)-\d+.*pdfa?$'),
     ('773__c', '773__w')),
...
def parse_filename(pdf_file):
    ...
    for (pattern_name, file_pattern, fields) in FILE_SEARCH_PATTERN:
        ...
        search_result = file_pattern.search(pdf_file)  # FIXME: make this work
        if search_result:
...
def process_pdf_dir(input_dir, file_wanted):
    ...
    for root, dirnames, filenames in os.walk(input_dir):
        for filename in fnmatch.filter(filenames, file_wanted):
...
def build_dicts(input_dir, file_wanted):
    """Create dictionaries from the TEI XML data."""
    for processed_pdf in process_pdf_dir(input_dir, file_wanted):
        rec_dict = {}
...
def build_marc_xml(input_dir, pubdate, separate=True, just_return=True, file_wanted="*.pdf", references_only=False, recid=None):
    """Build a MARCXML file from the HEPRecord dictionary."""
    counter = 0
    all_records = {}
    cnum = ''

    for bd in build_dicts(input_dir, file_wanted):
        dic, cnum = bd
        if not dic:
        ...
        if references_only:
            ...
            for ref in dic["references"]:
                ...
                journal_title = ref["journal_pubnote"].get("journal_title", "") or ""
                volume = ref["journal_pubnote"].get("journal_volume", "")
                pages = ref["journal_pubnote"].get("page_range", "")
                year = ref["journal_pubnote"].get("year", "")
                pubnote = u"{},{},{}".format(journal_title, volume, pages)
                # marcdict["999C5"].append({"s":pubnote, "y":year})
                if "worldscientific" in journal_title:
                    continue  # HACK: grobid thinks the copyright declaration is a reference
                marcdict["001"] = recid
                marcdict["999C5"].append({"p":journal_title, "v":volume, "c":pages, "y":year})
